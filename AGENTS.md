...
- 이미지·영상 파일은 기본적으로 `asset/` 아래에 둔다. 새 묶음 자산은 용도별 하위 폴더를 만들 수 있다. 예: `asset/studio/`.
- 이미지·영상 파일명이 한글이거나 공백/특수문자를 포함하면, 추가 또는 수정 작업 중 자동으로 영문 소문자 kebab-case 파일명으로 바꾼 뒤 `asset/` 아래로 이동한다.
